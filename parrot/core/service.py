...
class Service(object):
    ...
    def update(self, payload=None):
        ...
        service_endpoint = self.find(method=method, endpoint=endpoint, parameters=parameters)
    ...
    def add_endpoint(self, payload=None):
        ...
        service_endpoint = self.find(method=method, 
        ...
        if service_endpoint.error_code != 0:
            if endpoint_url in self.endpoints:
                ...
                service_endpoint = ServiceEndpoint(url=endpoint_url, parameters=parameters, description=description)
                self.endpoints[service_endpoint.url] = service_endpoint
                self.endpoints[service_endpoint.url].add_response(payload=payload)
            
            did_add = True
        else:
            message = f'The endpoint already exists: ({method}) {endpoint_url} {parameters}'
    ...
    def info(self, app_info=None):
        ...
        """
        payload   = {'name': self.name, 'lag': self.lag, 'fuzz': self.fuzz, 'description': self.description}
        service_endpoints = []
        ...
        if app_info:
            payload['version']   = f'{app_info.name} {app_info.version} ({app_info.build})'
    ...
    if config:
        with app.app_context():
            try:
                ...
                service     = _parse_bundle(config=config, 
                                            data=data, 
...
def _parse_bundle(config=None, data=None, config_path=None, responses_folder=None):
    ...
    if len(endpoint_configs) == 0:
        current_app.logger.info("{name} does not have any endpoints.".format(name=service.name))
    else:
        for endpoint_config in endpoint_configs:
            endpoint_url         = endpoint_config.get(API.CONFIGURATION_URL_KEY)
            endpoint_parameters  = endpoint_config.get(API.RESPONSE_PAYLOAD_PARAMETERS_KEY, None)
            endpoint_responses   = endpoint_config.get(API.CONFIGURATION_RESPONSES_KEY)
            endpoint_description = endpoint_config.get(API.CONFIGURATION_DESCRIPTION_KEY, '') 
            ...
            for method, content in endpoint_responses.items():
                try:
                    ...
                    if status_code != 204 and (content_file is None or len(content_file) == 0):
                        raise ValueError('The value of the `content` node is invalid for `status_code` of `%d`' % status_code)
                    ...
                    if status_code != 204:
                        ...
                        json_path     = os.path.join(config_path, config, responses_folder, content_file)
                        data          = json.load(open(json_path))
                        response_body = data
                    ...
                    payload = {
                        ...
                        API.RESPONSE_PAYLOAD_HEADERS_KEY    : content.get(API.RESPONSE_PAYLOAD_HEADERS_KEY, API.DEFAULT_HEADER),
                        API.RESPONSE_PAYLOAD_CONTENT_KEY    : response_body,
                        API.RESPONSE_PAYLOAD_LAG_KEY        : content.get(API.RESPONSE_PAYLOAD_LAG_KEY, API.DEFAULT_RESPONSE_LAG),
                        API.RESPONSE_PAYLOAD_FUZZ_KEY       : content.get(API.RESPONSE_PAYLOAD_FUZZ_KEY, False)
                    }
