...
def load_bundle(bundle_name):
    ...
    if bundle_name:
        ...
        if g.service is None:
            ...
            response.message     = BUNDLE_ERROR.MESSAGES[BUNDLE_ERROR.NOT_FOUND].format(bundle_name)
            response.status_code = API.HTTP_STATUS_CODE_NOT_FOUND
        else:
...
def update_endpoint(payload=None):
    ...
    response.status_code = API.HTTP_STATUS_CODE_NO_CONTENT if did_update else API.HTTP_STATUS_CODE_OK
    response.error_code  = API.RESPONSE_RESULT_SUCCESS if did_update else CONFIG_ERROR.COULD_NOT_UPDATE_ENDPOINT
...
def add_endpoint(payload=None):
    ...
    response             = ServiceResponse()
    response.headers     = API.DEFAULT_HEADER
    response.status_code = API.HTTP_STATUS_CODE_NO_CONTENT if did_add else API.HTTP_STATUS_CODE_OK
    response.error_code  = API.RESPONSE_RESULT_SUCCESS if did_add else CONFIG_ERROR.COULD_NOT_ADD_ENDPOINT
...
def reset_service():
    ...
    response             = ServiceResponse()
    response.headers     = API.DEFAULT_HEADER
    response.status_code = API.HTTP_STATUS_CODE_NO_CONTENT if did_reset else API.HTTP_STATUS_CODE_INTERNAL_SERVER_ERROR
